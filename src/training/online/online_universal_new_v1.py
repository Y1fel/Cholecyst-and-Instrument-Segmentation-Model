...
def streaming_online_loop(args, learner, video_fps=25):
    ...
    try:
        while True:
            ...
            # === 步骤3：训练模型（仅使用选中的帧） ===
            # 更新控制：Frame Selection已经筛选了关键帧，只需频率保护
            do_update = True
            update_reason = "frame_selected"

            if int(args.update_every_n) > 1:
                if learner.global_frame_idx % int(args.update_every_n) != 0:
                    do_update = False
                    update_reason = f"freq_protection(every_{args.update_every_n})"

            if not do_update:
                print(
                    f"[Step {learner.global_step}] 跳过更新: {update_reason} (已选中{len(selected_frames_for_training)}帧)")
