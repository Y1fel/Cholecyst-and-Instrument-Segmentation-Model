...
def parse_args():
    ...
    p.add_argument("--offline_num_classes", type=int, help="Teacher num classes")
...
class FrameSelector:
    ...
    def simplified_ssim(self, frame1, frame2, size=64):
        ...
        frame2_small = torch.nn.functional.interpolate(frame2.unsqueeze(0), size=(size, size), mode='bilinear').squeeze(0)
...
class OnlineLearner:
    def __init__(self, model, device, args, output_mgr, monitor,
        ...
        self.frame_selector = FrameSelector(threshold=args.frame_selector_threshold) if args.use_frame_selector else None
    ...
    def evaluate(self, data_loader):
        """origin 中的 evaluate（包装 Evaluator）"""
        self.model.eval()
        ...
        for (batch_idx, (images, _)) in enumerate(train_loader):
            ...
            if val_loader is not None and (self.global_step % self.args.eval_interval == 0):
                ...
                try:
                    ...
                    pass
                print(f"[Step {self.global_step}] Val loss: {val_metrics.get('val_loss', 0):.4f} | IoU: {val_metrics.get('iou', 0):.4f}")
...
def main():
    ...
    student_model = load_student_model(args.student_model_path, args.student_model_name, args.student_num_classes, device)
