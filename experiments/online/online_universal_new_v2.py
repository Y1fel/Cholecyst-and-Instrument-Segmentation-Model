...
def parse_args():
    ...
    p.add_argument("--no_use_frame_selector", action='store_false',dest='use_frame_selector', default=False)
    p.add_argument("--use_replay_buffer", action='store_true', default=True)
    p.add_argument("--no_use_replay_buffer", action='store_false',dest='use_replay_buffer', default=False)
    p.add_argument("--replay_capacity", type=int, default=700)
    ...
    p.add_argument("--no_stream_output", action='store_false',dest='stream_output', default=False)
    ...
    p.add_argument("--no_denoise_student_pred", action='store_false',dest='denoise_student_pred', default=False, help="关闭对student预测进行去噪处理")
...
def streaming_online_loop(args, learner, video_fps=25):
    ...
    try:
        while True:
            ...
            # === 训练频率控制：每隔200帧才允许训练一次 ===
            do_training = True
            training_reason = "allowed"

            if int(args.update_every_n) > 1:
                if all_frames_count % int(args.update_every_n) != 0:
                    do_training = False
                    training_reason = f"freq_protection(every_{args.update_every_n}_frames)"

            if not do_training:
                print(f"[Frame {all_frames_count}] 跳过训练: {training_reason}")
                continue
            ...
            # === 步骤3：训练模型（仅使用选中的帧） ===
            print(
                f"[Frame {all_frames_count}] 开始训练: {training_reason} (已选中{len(selected_frames_for_training)}帧)")
