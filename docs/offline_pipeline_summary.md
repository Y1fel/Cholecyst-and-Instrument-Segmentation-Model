...
This document summarizes the end-to-end offline training workflow implemented in `src/training/offli### 3.3 Compose mapping

`compose_mapping` com-- **Fo$$L_{total} = \alpGiven class-specific true positives (TP), false positives (FP), and false negatives (FN):
...
Background exclusion simply removes class id 0 from the per-class arrays before averaging.{2} \operatorname{KL}(\sigma(z_s/T) \parallel \sigma(z_t/T)) + \beta\, L_{task} + \gamma\, L_{feature}$$Loss**: $$L = \alpha (1 - p_{t})^{\gamma} L_{CE}$$, mitigating class imbalance.**CombinedLoss** (CE + Dice). Dice uses $$\mathrm{Dice}(p,t) = \frac{2|p \cap t| + \epsilon}{|p| + |t| + \epsilon}$$, while the total loss is $$L = (1 - \lambda) L_{CE} + \lambda L_{Dice}$$oses the watershed and classification mappings:
...
where $\psi$ is the scheme-specific mapping from base semantics to training labels. Custom dictionaries are normalised to integer keys/values, avoiding JSON string keys.line_universal.py`. This script serves as the unified entry point for standard training and knowledge distillation experiments on medical segmentation datasets.
...
The temperature factor $T^2$ in the KL term compensates for the gradient scaling effect of temperature division.function` builder selects among several criteria:
...
- **Advanced weights** (`compute_auto_class_weights`): More precise calculation with configurable sampling ratioombinedLoss** (CE + Dice). Dice uses $$\mathrm{Dice}(p,t) = \frac{2|p \cap t| + \epsilon}{|p| + |t| + \epsilon}$$, while the total loss is $$L = (1 - \lambda) L_{CE} + \lambda L_{Dice}$$
- **FocalLoss**: $$L = \alpha (1 - p_{t})^{\gamma} L_{CE}$$, mitigating class imbalance.ses the watershed and classification mappings:
...
where $\psi$ is the scheme-specific mapping from base semantics to training labels. Custom dictionaries are normalised to integer keys/values, avoiding JSON string keys.offline pipeline implemented in this repository. It covers data ingestion (with SEG8K-specific handling), label mapping, model interfaces, optimisation machinery, knowledge distillation, checkpoint management, and resume semantics. All descriptions are derived from the current source code so the document can serve as a technical appendix for reports or publications.

## 1. Offline Training Overview

The offline stage targets the SEG8K dataset and runs inside `src/training/offline/train_offline_universal.py`. The script unifies teacher (standard) training and student knowledge distillation (KD) training. Both flows share a common template:

1. Parse CLI arguments and merge overrides from YAML configs (`merge_config_with_args`).
2. Optionally resume from a checkpoint directory via `ResumeManager`.
3. Construct datasets with SEG8K-specific preprocessing (`SegDatasetMin`).
4. Derive class mappings and (optionally) automatic class weights.
5. Build the requested model through the `model_zoo`.
6. Train with `train_one_epoch` / `validate`, log metrics, run hybrid checkpoint logic, and trigger early stopping when needed.
7. For KD jobs, load the teacher, assemble `DistillationLoss`, and emit evidence packages.
8. Persist results via `OutputManager`, which centralises directory layouts, configs, metrics CSVs, and checkpoint naming.
...
\[arphi(w) = egin{cases}
    	ext{normalized"](custom_mapping) & 	ext{if provided}, \
    \psi(\mathrm{WATERSHED\_TO\_BASE\_CLASS}[w]) & 	ext{if } w
eq255, \
    255 & 	ext{otherwise}
\end{cases}\]

where `ψ` is the scheme-specific mapping. Custom dictionaries are normalised to integer keys/values, avoiding JSON string keys.
...
- **Cross-Entropy (CE)** with optional class weights.
- **CombinedLoss** (CE + Dice). Dice uses \( \mathrm{Dice}(p,t) = rac{2|p \cap t| + \epsilon}{|p| + |t| + \epsilon} \), while the total loss is \( L = (1 - \lambda) L_{CE} + \lambda L_{Dice} \).
- **FocalLoss**: \( L = lpha (1 - p_{t})^{\gamma} L_{CE} \), mitigating class imbalance.
- **LabelSmoothingCrossEntropy**: replaces the one-hot target distribution with a smoothed variant to improve calibration.
- **Automatic class weights** via `compute_auto_class_weights` (if composite losses are available) or legacy frequency-based weights (`compute_class_weights`).

For KD runs, `DistillationLoss` combines:

\[
L_{total} = lpha\, T^{2} \operatorname{KL}(\sigma(z_s/T) \parallel \sigma(z_t/T)) + eta\, L_{task} + \gamma\, L_{feature}
\]
...
- IoU: \( \mathrm{IoU} = rac{TP}{TP + FP + FN + \epsilon} \)
- Dice: \( \mathrm{Dice} = rac{2TP}{2TP + FP + FN + \epsilon} \)
- Accuracy: \( rac{TP + TN}{TP + FP + FN + TN + \epsilon} \)
